// Math Intermediate Diagnostic scoring modal (principal): same as Math Beginner but for
// PACEs 1049-1096; mark learning gaps, start PACE auto-fills to the lowest miss (editable).
...
// Math Intermediate PACE numbers: 1049–1096 (12 per row, 4 rows)
const MATH_INT_PACES = Array.from({ length: 48 }, (_, i) => 1049 + i);
const PACE_ROWS      = Array.from({ length: 4  }, (_, i) => MATH_INT_PACES.slice(i * 12, i * 12 + 12));
...
// Rendered by <RecordDiagnostic> (showMathIntermediate). onClose = () => setShowMathIntermediate(false);
// onSaved = handleMathIntSaved (closes the modal + reloads the student's diagnostics).
export default function MathIntermediateDiagnosticModal({ student, existing, onClose, onSaved }) {
      ...
      <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-4 sm:px-8 py-6 space-y-6">
          ...
          <div className="border border-outline-variant rounded-xl p-5 space-y-4">
            ...
            {/* PACE Grid — 4 rows × 12 cols (1049–1096); each cell -> toggleGap(pace) (also auto-fills startPace) */}
