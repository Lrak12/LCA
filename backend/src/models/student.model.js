...
export const findAll = () =>
  supabaseAdmin.from(TABLE).select("*, users(user_id, username, email, is_active), grade_level!student_gl_id_fkey(gl_id, level_name, sy_id)");

export const findById = (student_id) =>
  supabaseAdmin.from(TABLE).select("*, users(user_id, username, email, is_active), grade_level!student_gl_id_fkey(gl_id, level_name, sy_id)").eq("student_id", student_id).single();
